...
class Application_Model_Facebook {
     ...
     */
    const FACEBOOK_APP_ID = '171399162956345';
    /**
     * Facebook Secret String
     */
    const FACEBOOK_SECRET = '28831eb6c4212a5ebfa0ecf3369a631c';
    /**
     * Facebook Graph Url
     */
...
// Check if the user is authed
     ...
     * @return array
     */
    public function getUserFriends2($justUsingApp = false) {
        if (!$justUsingApp) {
            $url = self::FACEBOOK_GRAPH_URL . 'me/friends';
            return $this->_getFromGraph($url);
        } else {
            $fql = "SELECT uid FROM user WHERE has_added_app=1 and uid IN (SELECT uid2
FROM friend WHERE uid1 = {$this->fbData['user_id']})";
